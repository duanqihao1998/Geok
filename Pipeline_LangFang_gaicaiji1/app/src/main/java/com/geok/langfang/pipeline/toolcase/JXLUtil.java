...
public class JXLUtil {
	...
	public static void initExcel(String fileName, String[] colName, int[] widthArr) {
		...
		try {
			...
			sheet.addCell((WritableCell) new Label(0, 0, fileName, arial14format));// 表头设置完成
	 ...
	 *            excel单元格宽度
	 */
	public static <T> void dataToExcel(ByteArrayOutputStream os, List<T> objList,
			String[] fieldArr, String fileName, String[] colName, int[] widthArr) {
		format();// 先设置格式
		WritableWorkbook workbook = null;
		try {
			// WorkbookSettings setEncode = new WorkbookSettings(); // 设置读文件编码
	...
	public static <T> void writeObjListToExcel(List<T> objList, String fileName, String[] fieldArr,
			Context c) {
		if (objList != null && objList.size() > 0) {
			...
			try {
				...
				writebook.write();
				Toast.makeText(c, "成功导出至手机存储/Keypoint/Keypoint.xls", 1000).show();
